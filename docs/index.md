...
**ICE_ORCA_DRAGON** is the physics-computation side of SYMPOSIUM apostle #2. 47 Python scripts + 22 JSON results compute Cayley-Dickson algebra breaking (32D ↔ 64D), sedenion (16D) analysis with Der(S)=g₂ verification, Higgs doublet candidates from sedenion ZD pairs, S₁~S₇ structural proofs, and orbit / custodial / rep / queue investigations. Every result feeds a *science-feedback-loop* that classifies, applies Fitting Detection, runs Lakatos evaluation, and updates the KG via Bayesian update.
...
| `/Users/lagyeongjun/CD/MIND/metahumotonic/나는야_ice_orca_dragon.md` | User's primary mythology source (자칭, CHU 명단) |
| `/Users/lagyeongjun/CD/MIND/metahumotonic/얼어붙은_강물의_노래.md` | ICED 반복 + SSB 구원기도 canon |
...
|--------|--------|----------------|
| 42 sedenion ZD pairs | `prove_higgs_results.json` | confirmation (external: Lygeros 2006) |
| Der(S) = g₂ (14D) | `sedenion_g2_deep.py` | confirmation_local (no peer review yet) |
| Koide Q = 2/3 | `derive_dimensionless_results.json` | numerology HOLD |
...
| mp / mW = 3·256 | `verify_mp_mW_results.json` | numerology candidate (Fitting Detection pending) |
...
| Apostle #2 canon (mythology paper source) | `/Users/lagyeongjun/CD/MIND/metahumotonic/나는야_ice_orca_dragon.md` |
...
| sexvoid 형식 | `prove_higgs_ZD_doublet.py` 42 ZD pairs as void-symmetry candidates |
