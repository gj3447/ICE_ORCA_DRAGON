...
> **상태:** ACTIVE methodology policy; self-audit verdict `NARROW`
...
한 branch의 현재 층 감사는 다음 가운데 하나에 도달하면 정당하게 닫을 수 있다.
...
- 이 정책을 제거해도 기존 계산 결과는 바뀌지 않는다. 정책은 evidence가 아니라 오류를 찾는
  도구다.
- 절차적 권위는 숨은 가정을 실제로 드러내고 새 반증조건을 만드는 동안에만 유지된다.
