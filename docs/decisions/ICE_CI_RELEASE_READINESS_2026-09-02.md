...
Node 24, `npm ci`, uv 0.12.7 and `uv sync --locked`, hydrates exactly the
tracked Phase 44 LFS object, and runs `npm run graph:check`. The Python sync is
required because the control-plane suite exercises one bounded kernel-spawn
contract through the repository `.venv`; a Node-only checkout is not a valid
test environment. The graph check remains the repository's combined
TypeScript, test, ontology, SHACL, retrieval, competency, and agent-routing
check.
...
tag/ref endpoints on 2026-09-02:
...
| `astral-sh/setup-uv` | `20cfd1bf945f4377ade1205e4dbc17946fc9a30d` | v10.0.1 | https://github.com/astral-sh/setup-uv/tree/v10.0.1 |
...
The uv action, exact-version input, and locked sync follow Astral's GitHub
Actions integration guidance:
...
rules. uv is installed by a full-SHA action pin, selects Python 3.13, and syncs
only the committed lock before any test can spawn the kernel.
