...
is shown to have no continuum limit against a constant-principal reference,
whereas the canonical slice measure converges to the Jacobi/Van-Vleck datum
over the recorded refinements.
...
def assemble_discrete_control(
    ...
    *,
    ...
    result: dict[str, object] = {
        ...
        "field_negative_count": int(np.count_nonzero(np.linalg.eigvalsh(field_hessian) < 0)),
        "schur": schur,
    }
...
def exact_controls(audit: Audit) -> dict[str, object]:
    ...
    audit.exact(
        "P30.determinant.canonical_slice_measure_power",
        sp.simplify(sp.sqrt(metric_det / metric_det_0) - (scale / scale_0) ** 2)
        == 0,
        "the canonical midpoint measure contributes the exact per-slice factor (a_e/a_0)^2",
    )
...
def numerical_controls(audit: Audit) -> dict[str, object]:
    ...
    audit.numerical(
        ...
        and min(record["W_T"] for record in scan_records[:-1]) > 0,
    ...
    audit.numerical(
        "P30.determinant.canonical_measure_convergence",
        raw_ratios[-1] > 50.0
        ...
        "the naked ratio grows rapidly across the recorded cutoffs while the canonical slice measure converges to the endpoint Van-Vleck datum",
    )

    schur_values = [float(item["schur"]) for item in contour_controls]
    audit.numerical(
        "P30.schur.discrete_bulk_lapse_convergence",
        np.all(np.diff(schur_values) < 0)
        and abs(schur_values[-1] - curvature) < 5e-4,
        "the discrete field-lapse Schur complement converges to W_TT",
    )
    audit.numerical(
        ...
        "independent field and lapse rotations leave one negative direction at every tested cutoff",
    )
    audit.numerical(
    ...
    audit.numerical(
        ...
        "the canonical endpoint factor and lapse Gaussian give the conditional local-flat magnitude",
    )
...
def run() -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "claim_status": {
            ...
            "independent_field_and_lapse_Wick_rotations_are_sufficient": "CONTRADICTED_BY_THE_MIXED_HESSIAN",
            "the_canonical_measure_normalized_endpoint_determinant_has_a_continuum_limit": "SUPPORTED_NUMERICALLY_IN_THE_RECORDED_TIME_SLICING",
        ...
        "scope_guard": {
            "computed": [
                ...
                "canonical time-slice measure convergence to an endpoint Van-Vleck datum",
            ...
            "not_computed": [
                ...
                "the BFV ghost/gauge complex and the inhomogeneous graviton, scalar, fermion, and gravitino spectra",
        ...
        "next_calculation": (
            "freeze a finite regulated full joint phase-space cycle, track its determinant "
            "line through N=0, and compute all upward-thimble intersections before "
