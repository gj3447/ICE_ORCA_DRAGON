...
\(\phi=0\), \(W=D_\Phi W=D_SW=F^S=V=0\), so the endpoint is supersymmetric Minkowski in this
...
and the exact potential is the second expression in Eq. E193.  This is the
...
\(a(-t)=a(t)\), \(\phi(-t)=\phi(t)\), and \(H(-t)=-H(t)\).  This proves a regular
...
\(0.9728\pm0.0029\) after adding DESI BAO.  It is therefore the better
potential-level background candidate of the two, but “observationally
settled” would be too strong.
...
3. define the candidate CPT/Pin action on those perturbation modes and test
   whether it selects a unique Gaussian state; separately construct a
   minisuperspace/background wavefunction or seam path integral that could
   define a measure over \(\phi_0\);
4. only then include reheating, map to \(k_*\), and compare \((A_s,n_s,r)\) with
   likelihood data.

If the background wavefunction or seam measure leaves a continuous \(\phi_0\)
family without a predictive weighting, the construction has an existence
result but no initial-value prediction.  If that independent background gate
selects a normalizable state concentrated near the 50--60 e-fold window, that
would be the first genuinely new prediction from this route.
