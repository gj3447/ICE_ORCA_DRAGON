...
EXPECTED_INPUT_SHA256 = "90f7704c889f78b389ec7fb1d6fef1f5d85a2a5331fe80ffb7ea9af3b767b982"
...
def expected_caps() -> dict[str, int]:
    return {
        ...
        "symbolic_operations": 768, "compact_steps": 624,
...
def exact_audit(audit: Audit) -> None:
    ...
    wu = u * cq - uq * c
    wz = z * cq - zq * c
    audit.identity("rawc.q0tail.wronskian_identity", sp.diff(wu, u) * uq * cq + sp.diff(wu, uq) * (A0 + lam * a) * u + sp.diff(wu, c) * cq + sp.diff(wu, cq) * A0 * c + lam * a * u * c, "W(U,c)_Q=-lambda*a*U*c for the lambda-independent reference c.")
    audit.identity("rawc.q0tail.differentiated_wronskian_identity", sp.diff(wz, z) * zq * cq + sp.diff(wz, zq) * ((A0 + lam * a) * z + a * u) + sp.diff(wz, c) * cq + sp.diff(wz, cq) * A0 * c + a * u * c + lam * a * z * c, "W(Z,c)_Q=-(a*U*c+lambda*a*Z*c).")
    ...
    audit.identity("rawc.q0tail.cutoff_partition", 24 * sp.Rational(-1, 4) - (sp.Rational(-10) - sp.Rational(-4)), "The coarse cutoff partition is exact.")
    audit.identity("rawc.q0tail.cutoff_partition_refined", 48 * sp.Rational(-1, 8) - (sp.Rational(-10) - sp.Rational(-4)), "The refined cutoff partition is exact.")
    audit.identity("rawc.q0tail.cutoff_partition_deep", 32 * sp.Rational(-1, 4) - (sp.Rational(-12) - sp.Rational(-4)), "The deeper cutoff partition is exact.")
...
def main() -> None:
    ...
    if conventions["precision_ladder_decimal_digits"] != [80, 120] or conventions["compact_taylor_order"] != 12 or [(row["Q_cut"], row["segments"]) for row in conventions["cutoff_segment_ladder"]] != [("-10", 24), ("-10", 48), ("-12", 32)]:
    ...
    for label in records:
        for tier_index in range(2):
            ...
            audit.control(f"rawc.q0tail.{label}.tier{tier_index + 1}.minus10_refinement_overlap", all(intersect(coarse[key], refined[key]) is not None for key in ("g", "gp")), "The -10 cutoff 24/48-step completed functional enclosures overlap.")
            g_tail_decreases = (
                deeper["bt"].upper() < coarse["bt"].upper()
                if label != "lambda_zero"
                else deeper["bt"].upper() <= coarse["bt"].upper()
            )
            audit.control(f"rawc.q0tail.{label}.tier{tier_index + 1}.cutoff_overlap_and_tail_decrease", bool(all(intersect(coarse[key], deeper[key]) is not None for key in ("g", "gp")) and g_tail_decreases and deeper["bz"].upper() < coarse["bz"].upper()), "The completed -10 and -12 cutoff enclosures overlap and the analytic tail radii decrease (or stay exactly zero for the lambda-zero g tail) at the earlier cutoff.")
