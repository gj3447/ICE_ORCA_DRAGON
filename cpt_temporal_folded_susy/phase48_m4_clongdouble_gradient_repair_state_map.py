#!/usr/bin/env python3
"""Phase 48: clongdouble gradient-only state-map repair control.

Only the generated Phase-41 gradient callable is evaluated in NumPy
clongdouble and then projected once to complex128.  State formation,
contraction, integration, cap, residual, roots, and the complete u2 step ladder
remain fixed.  The program writes no files and performs no root search.
"""
...
INPUT_PATH = SCRIPT_PATH.with_name(
    "PHASE48_M4_CLONGDOUBLE_GRADIENT_REPAIR_STATE_MAP_INPUTS.json"
)
...
INPUT_COMMIT = "2ba73ec37408c09be10578a6b3a009d75e9408aa"
INPUT_SHA256 = "5604456ece8f542500cafd19b1429fe439514f780c2c8556195ddb934796f335"
PHASE42_INPUT_SHA256 = (
    "1cc88c489b5240019aaf339b25d0cebac9b4a1560b09cbec9c3079ce2067afb6"
)
RESULT_SCHEMA = "ice-phase48-clongdouble-gradient-repair-state-map/v1"
...
def progress(message: str) -> None:
    print(f"[Phase48] {message}", file=sys.stderr, flush=True)
...
def validate_inputs(manifest: Mapping[str, Any]) -> dict[str, Any]:
    if sha256_path(INPUT_PATH) != INPUT_SHA256:
        raise InvalidRun("Phase48 input manifest hash drift")
    if manifest.get("schema") != (
        "ice-phase48-clongdouble-gradient-repair-state-map-inputs/v1"
    ):
        raise InvalidRun("Phase48 input schema drift")
    if manifest.get("phase") != 48:
        raise InvalidRun("Phase48 input phase drift")
    scope = manifest["scope"]
    if scope["targets_in_order"] != ["shared_zero", "phi_plus", "a_plus"]:
        raise InvalidRun("Phase48 target order drift")
    if scope["central_difference_steps_in_order"] != [2.0e-6, 5.0e-7, 1.0e-7]:
        raise InvalidRun("Phase48 step order drift")
    if scope["perturbation_signs_in_order"] != [1, -1]:
        raise InvalidRun("Phase48 sign order drift")
    if scope["trajectory_sample_fractions_in_order"] != [0.0, 0.25, 0.5, 0.75, 1.0]:
        raise InvalidRun("Phase48 fraction order drift")
...
def hybrid_flow(model: Any, saddle: Any, fixed: Any, xi: np.ndarray) -> np.ndarray:
    xi64 = np.asarray(xi, dtype=np.complex128).reshape(7)
    u64 = np.asarray(fixed.linear_map @ xi64, dtype=np.complex128).reshape(7)
    w64 = np.asarray(saddle.saddle_w + u64, dtype=np.complex128).reshape(7)
    w_long = np.asarray(w64, dtype=np.clongdouble)
    ...
    gradient128 = np.asarray(gradient_long, dtype=np.complex128).reshape(7)
    flow = np.asarray(
        -np.conjugate(fixed.linear_map.T @ gradient128), dtype=np.complex128
    ).reshape(7)
...
def run() -> dict[str, Any]:
    ...
    progress("load pinned Phase 42 context and retained Phase 45-47 controls")
    phase42 = load_module("ice_phase42_for_phase48", PHASE42_SCRIPT_PATH)
    phase43 = load_module("ice_phase43_for_phase48", PHASE43_SCRIPT_PATH)
    ...
    for label in scope["targets_in_order"]:
        ...
        for step in steps:
            for sign in signs:
                ...
                progress(f"{label}: integrate gradient-only hybrid {slot}")
    ...
    result = {
        ...
        "interpretation_boundary": {
            ...
            "historical_phase41_phase45_phase46_phase47_results_unchanged": True,
            "gradient_only_platform_specific_ablation": True,
...
def main() -> int:
    try:
        ...
        emergency = result_with_self_digest(
            {
                ...
                "classification": "CLONGDOUBLE_GRADIENT_REPAIR_STATE_MAP_INCONCLUSIVE",
        ...
        print(f"Phase48 invalid run: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 1
