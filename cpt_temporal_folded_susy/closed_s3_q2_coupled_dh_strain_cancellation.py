...
def main():
 ...
 p=json.loads(raw); root=Path(__file__).resolve().parent.parent; pins=[pin(root,x) for x in p["upstream_results"]]
 ...
 div=-8*q2; hrr=sp.diff(q2,chi,2); grad=sp.diff(th,chi); f=grad**2
 strain=z.direct_integral(q1*(-xi**2*div/(2*a**3)+a*f*div/2-a*hrr*f),chi)
 hz=z.direct_integral(q1*q2*(-3*xi**2/a**3+a*f)/2,chi)
 he=z.direct_integral(-a*q1*hrr*f,chi)
 dg=sp.simplify(sp.Rational(8,3)*hz-he)
 ck(rows,"CS3Q2.metric.action.zeta",-sp.Rational(8,3)+sp.Integer(8)/3,"D_g gives delta zeta2=-8/3.")
 ...
 ok=all(x["passed"] for x in rows); res={"schema_version":"ice.closed-s3-q2-coupled-dh-strain-cancellation.result.v1","calculation_id":p["calculation_id"],"numbered_phase":None,"run_status":"VALID_RUN","verdict":"KEEP_Q2_COUPLED_DH_STRAIN_CANCELLATION_NOT_FULL_ADM_HDA" if ok else "KILL_Q2_COUPLED_DH_PACKET","input_manifest":{"path":"cpt_temporal_folded_susy/"+I,"sha256":hs},"upstream_results":pins,"primary_sources":p["primary_sources"],"exact_checks":rows,"check_summary":{"exact_passed":sum(x["passed"] for x in rows),"exact_total":len(rows),"all_executable_checks_passed":ok},"rows":out,"computed_scope":"selected Q2 metric-generator cancellation of selected fixed-metric scalar-matter DH strain only","non_claims":["gravitational Hamiltonian constraint","DD or HH brackets","full HDA or Jacobi","BFV/anomaly/physics"],"runner":{"path":"cpt_temporal_folded_susy/closed_s3_q2_coupled_dh_strain_cancellation.py","sha256":sh(Path(__file__).read_bytes())},"environment":{"python":platform.python_version(),"sympy":sp.__version__}}
 res["result_payload_sha256_without_self"]=sh(cb(res)); Path(__file__).with_name(R).write_bytes(cb(res)+b"\n"); print("CLOSED_S3_Q2_COUPLED_DH_STRAIN_CANCELLATION_RESULT="+json.dumps({"run_status":res["run_status"],"verdict":res["verdict"],"exact_passed":res["check_summary"]["exact_passed"],"exact_total":res["check_summary"]["exact_total"],"result":R},sort_keys=True))
