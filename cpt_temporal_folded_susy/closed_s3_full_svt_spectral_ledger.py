...
EXPECTED_INPUT_SHA256 = "e1381c2f589c6ada83a3c04401fb7bb232daf1bf8a081f280e14fc7421b13b03"
...
class Ledger:
    ...
    def guard(self, guard_id: str, theorem: str, hypotheses: str, conclusion_and_scope: str) -> None:
        ...
        self.theorem_guards.append({"id": guard_id, "verified": True, "verification_mode": "SOURCE_PIN_AND_ANALYTIC_HYPOTHESIS_SCOPE_AUDIT_NOT_AN_EXECUTABLE_PROOF", "theorem": theorem, "hypotheses": hypotheses, "conclusion_and_scope": conclusion_and_scope})
...
def expected_low_mode_rows() -> dict[str, list[dict[str, int]]]:
    return {
        "scalar": [
            {"n": 0, "rough_eigenvalue": 0, "degeneracy": 1},
            {"n": 1, "rough_eigenvalue": 3, "degeneracy": 4},
            {"n": 2, "rough_eigenvalue": 8, "degeneracy": 9},
            {"n": 3, "rough_eigenvalue": 15, "degeneracy": 16},
        ],
        "transverse_vector": [
            {"n": 1, "rough_eigenvalue": 2, "hodge_eigenvalue": 4, "combined_degeneracy": 6},
            {"n": 2, "rough_eigenvalue": 7, "hodge_eigenvalue": 9, "combined_degeneracy": 16},
            {"n": 3, "rough_eigenvalue": 14, "hodge_eigenvalue": 16, "combined_degeneracy": 30},
        ],
        "tt_tensor": [
            {"n": 2, "rough_eigenvalue": 6, "declared_lichnerowicz_eigenvalue": 12, "combined_degeneracy": 10},
            {"n": 3, "rough_eigenvalue": 13, "declared_lichnerowicz_eigenvalue": 19, "combined_degeneracy": 24},
            {"n": 4, "rough_eigenvalue": 22, "declared_lichnerowicz_eigenvalue": 28, "combined_degeneracy": 42},
        ],
    }
...
def read_input() -> tuple[dict[str, Any], str]:
    ...
    if payload["declared_conventions"]["cutoffs"] != [2, 3, 5, 8] or payload["required_low_mode_rows"] != expected_low_mode_rows():
        raise AssertionError("cutoff or low-mode convention drift")
...
def run(payload: dict[str, Any], input_sha: str) -> dict[str, Any]:
    ...
    ledger.guard("CS3SVT.guard.source_scalar_sector", "unit-round-S3 scalar harmonic spectrum and degeneracy", "Delta_rough=-D^2, n is an integer with n>=0, and scalar modes use the pinned Lindblom-Taylor-Zhang convention", "eigenvalue lambda_n=n(n+2) and degeneracy (n+1)^2 are imported as source-pinned spectral data; this runner does not prove scalar-basis completeness")
    ledger.guard("CS3SVT.guard.source_transverse_vector_sector", "unit-round-S3 transverse-vector spectrum and combined degeneracy", "D^a V_a=0, n is an integer with n>=1, both chiral families are combined, and the pinned source convention is used", "rough eigenvalue lambda_n-1, Hodge eigenvalue lambda_n+1, and combined degeneracy 2n(n+2) are imported; no explicit vector basis or chirality-resolved ledger is constructed")
    ledger.guard("CS3SVT.guard.source_tt_sector", "unit-round-S3 transverse-traceless rank-two spectrum and combined degeneracy", "D^aT_ab=0=T^a_a, n is an integer with n>=2, both TT families are combined, and the pinned Lindblom/Higuchi conventions including the cited erratum are used", "rough eigenvalue lambda_n-2 and combined degeneracy 2(n-1)(n+3) are imported; the polynomial is never applied below n=2 and no TT completeness proof is produced")
    ledger.guard("CS3SVT.guard.derived_harmonic_identities", "Bochner/commutator identities for scalar- and vector-derived harmonics on unit S3", "Ric_ab=2 gamma_ab, source modes are normalized, and the stated nonexceptional ranges n>=1 or n>=2 are respected", "the gradient, tracefree-Hessian and transverse-symmetrized-gradient norms and rough shifts recorded below are source-pinned analytic identities, not results proved by symbolic self-comparison")
    ledger.guard("CS3SVT.guard.low_mode_geometry", "constant, conformal-scalar and Killing low-mode identities on unit S3", "the source-pinned harmonic families and their allowed n ranges are used", "n=0 scalar gradient, n=1 scalar tracefree Hessian, and n=1 transverse-vector symmetrized gradient vanish; n=0 transverse-vector and n<2 TT sectors are absent by the allowed family ranges")

    scalar_rows = [{"n": mode, "rough_eigenvalue": int(lam.subs(n, mode)), "degeneracy": int(d_scalar.subs(n, mode))} for mode in range(4)]
    vector_rows = [{"n": mode, "rough_eigenvalue": int((lam - 1).subs(n, mode)), "hodge_eigenvalue": int((lam + 1).subs(n, mode)), "combined_degeneracy": int(d_vector.subs(n, mode))} for mode in range(1, 4)]
    tt_rows = [{"n": mode, "rough_eigenvalue": int((lam - 2).subs(n, mode)), "declared_lichnerowicz_eigenvalue": int((lam + 4).subs(n, mode)), "combined_degeneracy": int(d_tt.subs(n, mode))} for mode in range(2, 5)]
    ledger.check("CS3SVT.transport.scalar_low_modes", scalar_rows == payload["required_low_mode_rows"]["scalar"], "The implemented scalar formulas reproduce the independently pinned n=0..3 table.")
    ledger.check("CS3SVT.transport.vector_low_modes", vector_rows == payload["required_low_mode_rows"]["transverse_vector"], "The implemented transverse-vector formulas reproduce the independently pinned n=1..3 table.")
    ledger.check("CS3SVT.transport.tt_low_modes", tt_rows == payload["required_low_mode_rows"]["tt_tensor"], "The implemented TT formulas reproduce the independently pinned n=2..4 table without evaluating the degeneracy polynomial outside its domain.")
    ledger.check("CS3SVT.general.vector_hodge_shift", sp.expand((lam - 1 + 2) - (lam + 1)) == 0, "The declared one-form Hodge shift transports the rough spectrum to lambda_n+1.")
    ledger.check("CS3SVT.general.tracefree_lichnerowicz_curvature_shift", sp.Integer(2) + sp.Integer(2) + sp.Integer(2) == 6, "With the pinned Riemann sign and tracefree definition, the two Ricci contractions and -2 Riemann contraction contribute 2+2+2=6.")
    ledger.check("CS3SVT.general.tt_lichnerowicz_shift", sp.expand((lam - 2 + 6) - (lam + 4)) == 0, "The explicitly declared tracefree-tensor Lichnerowicz definition transports the TT rough eigenvalue to lambda_n+4.")
    ...
    ledger.check("CS3SVT.derived.gradient_norm_low_modes", [int(gradient_raw_norm.subs(n, mode)) for mode in range(1, 5)] == [3, 8, 15, 24], "The declared raw-gradient norm polynomial transports to [3,8,15,24] at n=1..4, hence its normalized form is defined there.")
    ...
    ledger.check("CS3SVT.derived.tf_hessian_norm_low_modes", [sp.simplify(tf_norm.subs(n, mode)) for mode in range(1, 4)] == [0, sp.Rational(80, 3), 120], "The source-pinned tracefree-Hessian norm polynomial transports to 0,80/3,120 at n=1,2,3.")
    ...
    ledger.check("CS3SVT.derived.transverse_sym_gradient_norm_low_modes", [int(vector_gradient_norm.subs(n, mode)) for mode in range(1, 4)] == [0, 10, 24], "The source-pinned transverse symmetrized-gradient norm polynomial transports to 0,10,24 at n=1,2,3.")
    ...
    ledger.check("CS3SVT.mode.n0.gradient_norm_zero", gradient_raw_norm.subs(n, 0) == 0, "At n=0 the source-pinned gradient norm polynomial is zero; the geometric constant-mode statement remains a theorem guard.")
    ledger.check("CS3SVT.mode.n0.transverse_vector_outside_domain", 0 not in range(1, 5), "The declared transverse-vector family starts at n=1; its degeneracy polynomial is not used at n=0.")
    ledger.check("CS3SVT.mode.n1.tf_hessian_norm_zero", sp.simplify(tf_norm.subs(n, 1)) == 0, "At n=1 the source-pinned tracefree-Hessian norm polynomial is zero.")
    ledger.check("CS3SVT.mode.n1.vector_sym_gradient_norm_zero", sp.simplify(vector_gradient_norm.subs(n, 1)) == 0 and d_vector.subs(n, 1) == 6, "At n=1 the source-pinned vector-derived norm polynomial is zero and combined transverse-vector degeneracy is six; Killing character remains a theorem guard.")
    for mode in (0, 1):
        ledger.check(f"CS3SVT.mode.n{mode}.tt_outside_domain", mode not in range(2, 5), "The declared TT family starts at n=2; its degeneracy polynomial is not evaluated for this absent mode.")
    for mode in range(2, 5):
        ledger.check(f"CS3SVT.mode.n{mode}.tt_positive_in_domain", d_tt.subs(n, mode) > 0, "Within the declared n>=2 TT domain, the combined degeneracy is positive.")
    ...
    result: dict[str, Any] = {
        ...
        "input_manifest": {"path": INPUT_RELPATH, "sha256": input_sha}, "upstream_results": upstream, "primary_sources": payload["primary_sources"], "declared_conventions": payload["declared_conventions"], "exact_checks": ledger.exact, "theorem_guards": ledger.theorem_guards,
        "check_summary": {"exact_passed": sum(item["passed"] for item in ledger.exact), "exact_total": len(ledger.exact), "theorem_guard_count": len(ledger.theorem_guards), "all_executable_checks_passed": exact_pass},
        "low_mode_transport_rows": {"scalar": scalar_rows, "transverse_vector": vector_rows, "tt_tensor": tt_rows},
        "allowed_mode_ranges": {"scalar": "n>=0", "transverse_vector": "n>=1", "tt_tensor": "n>=2", "polynomials_outside_declared_ranges": "NOT_EVALUATED_AS_SECTOR_DEGENERACIES"},
        ...
        "computed_scope": "source-pinned convention transport plus unit-S3 low-mode and cumulative-count arithmetic for all three SVT sectors; FULL_SVT does not mean an explicit basis or completeness proof", "required_fail_closed_outputs": expected_nulls(),
