...
- When the user asks for scientific intuition, use `./ice intuition search "<question>" --target intuition::<topic:id> --json` for a discussion-level sidecar topic, or `--target cpt::<open:id>` when an existing canonical open problem has matching scope. A topic response has `canonical_target: null` and `canonical_context: null`; never fabricate a canonical target merely to obtain GraphRAG context. Treat `non_authoritative_signals` and topic links only as source-linked question lenses. Their derived links support navigation; they do not merge the sidecar into the canonical graph. Read the cited primary source and let a human retain at most one bounded falsifiable question; only a separately reviewed canonical blocker question may return to `./ice agent plan`. Never feed a signal directly into a runner or durable handoff.
