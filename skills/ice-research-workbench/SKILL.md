...
- Before accepting a material navigation or retrieval change, run `./ice graphrag eval --limit 12 --json` and `./ice graphrag diff --base HEAD --limit 12 --json` after `./ice ontology review`. The fixed suite controls canonical-locator retrieval only; inspect each rank/pass movement with the native graph and source context. Do not add a case for every experiment.
