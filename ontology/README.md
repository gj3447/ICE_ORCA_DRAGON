...
`families` verifies the six PARTIAL corpus roots as research families, separates generated bytecode
from ordinary files, and checks decisive-result bundles against existing evidence nodes. It adds
provenance density without upgrading PARTIAL coverage or manufacturing file-level evidence.
...
Use `./ice bridges summary --json` for the separately pinned resolution audit. The 2026-09-03 audit
found zero safe UID matches among the 75 unresolved keys; 23 old SYMPOSIUM phase-number candidates are
explicit semantic collisions. The audit is a noncanonical read-only sidecar and cannot mutate these
statuses.
