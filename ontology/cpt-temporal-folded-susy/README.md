...
`2026-08-31T16:16:05Z`; 1383 nodes, 3621 edges). Run and analytic-evidence details live in the
...
verifies 384/384 stored hashes.
...
Wronskian step now gives only the endpoint margin
\(\partial_\kappa\rho(Q_0)<-\kappa_{\rm left}/980<0\); the reference-state variation and complete
kappa-differentiated minus tail needed for \(\partial_\kappa g\) remain open. These results are not an absolute actual-\(\Gamma_1\)
...
| What is the current priority, and did priority 1 move? | Yes. The Bessel-anchored route reaches \(Q_0\), the complete normalized rotating-tail calculation encloses \(g=\Gamma_1/u(Q_0)\), and the correlated sign strip gives at least one selected declared-boundary zero per lambda in \([-10^{-4},10^{-4}]\). The strict-negative kappa chain now reaches the selected projective endpoint at \(Q_0\) with \(\partial_\kappa\rho(Q_0)<-\kappa_{\rm left}/980\), but it is not \(\partial_\kappa g\). The next P1 blocker is the reference-state kappa variation plus the complete kappa-differentiated minus tail; only after that may transversality, uniqueness/selector or continuation/velocity be tested as separate questions. Absolute amplitude/orientation, global roots, nonreal endpoint/Weyl data, RAQ and \(C/H\) equivalence remain open. | `claim:RAW_C_Q0_NORMALIZED_DIFFERENTIATED_ROTATING_TAIL_GAMMA1_FUNCTIONAL_HOLDS`; `claim:RAW_C_CORRELATED_KAPPA_LAMBDA_GAMMA1_SIGN_STRIP_HAS_AT_LEAST_ONE_DECLARED_BOUNDARY_ZERO_PER_LAMBDA`; `claim:RAW_C_PLUS_ENDPOINT_KAPPA_PROJECTIVE_SENSITIVITY_ANCHOR_HOLDS`; `claim:RAW_C_QPLUS_TO_QSWITCH_KAPPA_PROJECTIVE_SENSITIVITY_TRANSPORT_HOLDS`; `claim:RAW_C_QSWITCH_TO_Q0_KAPPA_PROJECTIVE_SENSITIVITY_SIGN_TRANSPORT_HOLDS`; `open:gate1-v0-raw-c-differentiated-tail-node-safe-transport`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
