...
| What is the current priority, and did priority 1 move? | Priority 1 is the raw-\(C\) differentiated plus-tail and node-safe \(F,F_\lambda\) bridge. One narrow part moved: at exactly \(\lambda=0\), an exact modified-Bessel direction and five disjoint real sign-changing existence brackets now have ball certificates. The differentiated tail datum, \(F_\lambda\), root uniqueness/completeness, nonreal spectral data, RAQ and \(C/H\) equivalence remain open. The metacognitive priority map records assumptions, fail-closed conditions and dependencies for P1--P7. | `concept:gate1-v0-six-bridge-metacognitive-priority-map`; `concept:raw-c-lambda-zero-bessel-ball-transport-anchor`; `claim:RAW_C_LAMBDA_ZERO_BESSEL_BALL_TRANSPORT_ANCHOR_HOLDS`; `open:gate1-v0-raw-c-validated-real-transport-and-flambda`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
