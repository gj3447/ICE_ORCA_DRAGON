...
| What is the current priority, and did priority 1 move? | Priority 1 is now the raw-\(C\) node-safe sensitivity transport and later normalized \(F,F_\lambda\) bridge. Two narrow pieces are closed at exactly \(\lambda=0\): the exact modified-Bessel direction with five real sign-changing existence brackets, and the normalization-invariant \(h(4)=\partial_\lambda[-u_Q/u]_0\) enclosure on every one of those brackets. Node-safe \(Q=4\to-4\) transport, normalized \(F_\lambda\), root uniqueness/completeness, nonreal spectral data, RAQ and \(C/H\) equivalence remain open. | `concept:gate1-v0-six-bridge-metacognitive-priority-map`; `concept:raw-c-lambda-zero-bessel-ball-transport-anchor`; `concept:raw-c-lambda-zero-differentiated-plus-tail`; `claim:RAW_C_LAMBDA_ZERO_DIFFERENTIATED_PLUS_TAIL_H_QPLUS_HOLDS`; `open:gate1-v0-raw-c-differentiated-tail-node-safe-transport`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
