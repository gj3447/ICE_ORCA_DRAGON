...
| How far does the \(\mathcal U_+\) spectral-to-\(m=2\) BFV chain go? | The selected densitized \(H\) has its scoped KL/RAQ and full-\(p\) two-branch regular completion. A separate declared weighted raw-\(C\) candidate has only a fixed-\(p\) endpoint/domain classification: \(Q=-\infty\) is limit-circle, \(Q=+\infty\) limit-point, and each fiber has indices \((1,1)\), so a boundary line is extra data. The exact selected-\(H\) spectral-coordinate map now reaches only the support-restricted target \(c<3p^2\); an explicit positive-measure complement blocks a \(p\)-preserving exact intertwiner to the declared unrestricted \(M_c\) target. The finite ordered \(A_0\oplus A_1\) BFV check transports a relative Pfaffian orientation only on \(\lambda\in[1/2,2]\), not an absolute measure or contour. The finite \(S^3\) zonal \(L=2\) projection ledger is a classical cutoff/remainder result, not HDA closure or a quantum anomaly calculation. No measurable global raw domain, raw rigging map or \(C/H\) equivalence was constructed. The endpoint state transform, lapse contour, absolute measure, anomaly, empirical map, physics and TOE remain open. | `claim:G1_V0_SELECTED_H_FULL_P_REGULAR_RAQ_COMPLETES_TO_TWO_LOG_BRANCHES_WITH_NO_INHERITED_ORIGIN_ATOM`; `claim:G1_V0_DECLARED_WEIGHTED_RAW_C_FIBERS_REQUIRE_EXTENSION_DATA`; `concept:gate1-v0-selected-h-support-restricted-spectral-intertwiner`; `claim:G1_V0_SELECTED_H_EXACT_SPECTRAL_INTERTWINER_REQUIRES_SUPPORT_RESTRICTED_TARGET`; `claim:G1_V0_SELECTED_H_HAS_P_PRESERVING_EXACT_INTERTWINER_TO_UNRESTRICTED_MC_TARGET`; `concept:gate1-v0-finite-bfv-pfaffian-orientation-transport`; `claim:G1_V0_FINITE_POSITIVE_LAMBDA_ODD_PFAFFIAN_RELATIVE_ORIENTATION_TRANSPORTS`; `concept:gate1-v0-closed-s3-scalar-projection-remainder-ledger`; `claim:G1_V0_CLOSED_S3_L2_ZONAL_SUBSPACE_IS_PRODUCT_CLOSED`; `claim:G1_V0_CLOSED_S3_L2_PROJECTED_ASSOCIATOR_IS_EXACT_DISCARDED_REMAINDER`; `open:gate1-v0-raw-constraint-rescaling-and-p-zero-completion`; `open:gate1-v0-lapse-modulus-contour-and-absolute-bfv-measure` |
