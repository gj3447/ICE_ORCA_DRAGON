...
| What is the current priority, and did priority 1 move? | Yes. The sharp transfer's six scale-free \(\Gamma_1\) proxy widths remain zero-containing, but the Bessel-anchored route now has a finite four-state \(Q_{\rm switch}\)-to-\(Q_0\) transfer with positive local \(s(Q_0)\) on both punctured boxes. It does not propagate a closed-theta MVT quotient or establish Q0 lambda-side separation. The next P1 blocker is a separate differentiated rotating-frame complete declared minus-tail \(\Gamma_1\) functional; panel-specific absolute-rho Picard tubes are required only if that calculation needs them. Root continuation/velocity, nonreal endpoint/Weyl data, RAQ and \(C/H\) equivalence remain open. | `claim:RAW_C_BESSEL_PRECONDITIONED_NODE_SAFE_Q0_PROJECTIVE_SENSITIVITY_TRANSFER_HOLDS`; `concept:raw-c-panelwise-picard-sensitivity-admission`; `open:gate1-v0-raw-c-differentiated-tail-node-safe-transport`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
