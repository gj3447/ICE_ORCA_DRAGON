...
`2026-08-31T15:39:49Z`; 1378 nodes, 3613 edges). Run and analytic-evidence details live in the
...
verifies 383/383 stored hashes.
...
selected projective derivative to \(Q_{\rm switch}\) with a strict-negative enclosure. It has not
been transported onward to \(Q_0\) or the complete \(\partial_\kappa g\). These results are not an absolute actual-\(\Gamma_1\)
...
| What is the current priority, and did priority 1 move? | Yes. The Bessel-anchored route reaches \(Q_0\), the complete normalized rotating-tail calculation encloses \(g=\Gamma_1/u(Q_0)\), and the correlated sign strip gives at least one selected declared-boundary zero per lambda in \([-10^{-4},10^{-4}]\). The strict-negative kappa chain now reaches \(Q_{\rm switch}\), but remains a selected projective derivative only. The next P1 blocker is the independent \(Q_{\rm switch}\)-to-\(Q_0\) actual-state kappa transport; reference variation and the complete differentiated tail are later separate requirements before \(\partial_\kappa g\), uniqueness/selector or continuation/velocity. Absolute amplitude/orientation, global roots, nonreal endpoint/Weyl data, RAQ and \(C/H\) equivalence remain open. | `claim:RAW_C_Q0_NORMALIZED_DIFFERENTIATED_ROTATING_TAIL_GAMMA1_FUNCTIONAL_HOLDS`; `claim:RAW_C_CORRELATED_KAPPA_LAMBDA_GAMMA1_SIGN_STRIP_HAS_AT_LEAST_ONE_DECLARED_BOUNDARY_ZERO_PER_LAMBDA`; `claim:RAW_C_PLUS_ENDPOINT_KAPPA_PROJECTIVE_SENSITIVITY_ANCHOR_HOLDS`; `claim:RAW_C_QPLUS_TO_QSWITCH_KAPPA_PROJECTIVE_SENSITIVITY_TRANSPORT_HOLDS`; `open:gate1-v0-raw-c-differentiated-tail-node-safe-transport`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
