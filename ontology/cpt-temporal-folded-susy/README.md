...
| What is the current priority, and did priority 1 move? | Priority 1 has moved to the nonzero-\(\lambda\) minus-end \(\Gamma_1\) functional and only then the declared \(F,F_\lambda\) bridge. Three narrow pieces are closed at exactly \(\lambda=0\): the exact modified-Bessel direction with five real sign-changing existence brackets, the normalization-invariant \(h(4)=\partial_\lambda[-u_Q/u]_0\) enclosure, and a direct smooth Green construction of positive \(J(-4)=-W=u^2h\) plus endpoint-only positive \(h(-4)\) on every full bracket. The last item is not numerical propagation of the \(h(4)\) state. Nonzero-\(\lambda\) \(\Gamma_1\), declared \(F_\lambda\)/root velocity, root uniqueness/completeness, nonreal spectral data, RAQ and \(C/H\) equivalence remain open. | `concept:gate1-v0-six-bridge-metacognitive-priority-map`; `concept:raw-c-lambda-zero-bessel-ball-transport-anchor`; `concept:raw-c-lambda-zero-differentiated-plus-tail`; `concept:raw-c-lambda-zero-node-safe-green-endpoint-construction`; `claim:RAW_C_LAMBDA_ZERO_NODE_SAFE_GREEN_WRONSKIAN_ENDPOINT_CONSTRUCTION_HOLDS`; `open:gate1-v0-raw-c-differentiated-tail-node-safe-transport`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
