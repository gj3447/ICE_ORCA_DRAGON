...
declared measurable \(p\)-preserving \(\Gamma_1=0\) raw-\(C\) self-adjoint extension, plus a finite
\(M_{\rm cut}\) proxy and one fixed-\(p=0\) nonreal branch precondition. The bridge design fixes the
selected-extension Herglotz target to \(m=-1/(2\hbar^2M)\) after an actual endpoint ratio exists; it
does not construct that endpoint, a spectral measure, the \(p\to0\) threshold or raw-\(C\) RAQ. The
...
| What is the current priority, and did priority 1 move? | Yes. The theorem/scope composition is now closed: on the exact current rectangle there is exactly one kappa-transverse normalized-\(G\) zero per fixed lambda and the roots form a continuous selector. P1's remaining optional upgrade is joint-\(C^1\) control, aligned \(\partial_\lambda G\) and velocity; the global root census and absolute amplitude/orientation remain separate. The more consequential independent blocker is P4's actual complex endpoint theorem. One measurable selected raw-\(C\) extension already exists, so the ordered work is now endpoint \(M\) and selected \(m=-1/(2\hbar^2M)\), compact nonzero-\(p\) bands, fiber measure/transform, \(p\to0\) threshold, global raw-\(C\) RAQ, and only then \(C/H\) equivalence. | `claim:RAW_C_FIXED_LAMBDA_ROOT_THEOREM_COMPOSITION_UNIQUE_TRANSVERSE_CONTINUOUS_SELECTOR_HOLDS`; `concept:raw-c-constant-boundary-direct-integral`; `concept:raw-c-weyl-spectral-raq-bridge-design`; `open:raw-c-fixed-box-nonreal-endpoint-certificate`; `open:gate1-v0-raw-constraint-rescaling-and-p-zero-completion`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
