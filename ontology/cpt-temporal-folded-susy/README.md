...
| How far does the \(\mathcal U_+\) spectral-to-\(m=2\) BFV chain go? | The selected densitized \(H\) has its scoped KL/RAQ and full-\(p\) two-branch regular completion. The raw-\(C\) branch has one **declared** parity-invariant, \(p\)-preserving measurable direct-integral self-adjoint extension from \(\Gamma_{1,p}=0\); the line remains extra quantization data. For root bracket 1, the new scale-free interval-Taylor calculation transports every inherited barrier-admissible switch state to \(Q=-4\), excludes a node there and closes the complete quotient tail on both punctured \(\lambda\) boxes. Its six widths are about \(1.2032\)–\(1.2034\), all miss the fixed \(1/4\) gate and contain zero, so sharp actual-direction transport, continuation, raw spectral resolution/RAQ, relevant \(p\)-mixing alternatives and \(C/H\) equivalence remain open. On closed \(S^3\), the exact linear scalar momentum generator \(D_L^{(1)}=\sum L_n(\Pi_{E,n}-\lambda_n\Pi_{\zeta,n}/3)\) is now fixed for the declared packet; Hamiltonian constraints, \(DD/DH/HH\), Jacobi, BFV and anomaly remain open. The selected-\(H\) endpoint, finite relative Pfaffian, absolute-BFV, empirical, physics and TOE boundaries are unchanged. | `claim:RAW_C_BARRIER_ADMISSIBLE_FINITE_OUTER_TRANSFER_HOLDS`; `evidence:raw-c-actual-nonzero-lambda-hybrid-validated-transfer-result`; `claim:CLOSED_S3_LINEAR_SCALAR_ADM_MOMENTUM_CONSTRAINT_GENERATOR_HOLDS`; `evidence:closed-s3-linear-scalar-adm-momentum-constraint-ledger-result`; `open:gate1-v0-raw-c-differentiated-tail-node-safe-transport`; `open:gate1-v0-classical-s3-hda-closure`; `open:gate1-v0-lapse-modulus-contour-and-absolute-bfv-measure` |
...
| What is the current priority, and did priority 1 move? | Yes. The actual family and complete minus-tail existence gap is no longer the first blocker: the full inherited \(\rho\in[-1,1]\) switch box has a validated finite scale-free transfer, positive \(v(-4)\), finite tail and exact \(\lambda=0\) inclusion regression. The aggregate certification still fails because every width is about \(1.203\), not below \(1/4\). Priority 1 is now a sharp certified \(Q=4\to Q_s\) enclosure of the **actual** Riccati direction, substantially narrower than \([-1,1]\), followed by the already tested compact/tail map. More digits or segments alone are not supported as the remedy. Root continuation/velocity, uniqueness/completeness, nonreal spectral data, RAQ and \(C/H\) equivalence remain open. | `concept:gate1-v0-six-bridge-metacognitive-priority-map`; `concept:raw-c-actual-nonzero-lambda-hybrid-validated-transfer`; `evidence:raw-c-actual-nonzero-lambda-hybrid-validated-transfer-result`; `open:gate1-v0-raw-c-differentiated-tail-node-safe-transport`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
