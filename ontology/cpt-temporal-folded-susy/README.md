...
\(g=\Gamma_1/u(Q_0)\) and its local lambda derivative. The two punctured-box \(g\) enclosures still
contain zero while their local derivative enclosures are strictly negative. This is not an absolute
actual-\(\Gamma_1\) amplitude/sign result, a correlated root curve, or a panel-specific absolute-rho
Picard tube. P4 has one fixed-box nonreal
...
| What is the current priority, and did priority 1 move? | Yes. The Bessel-anchored route now reaches \(Q_0\), and the separate complete normalized rotating-tail calculation encloses \(g=\Gamma_1/u(Q_0)\) and its local derivative. The derivative is negative on both punctured product boxes, but every \(g\) enclosure contains zero. The next P1 blocker is therefore a separately justified correlated parameter/root localization and, where needed, absolute amplitude/orientation control—not another copy of the completed normalized-tail calculation. Root existence/continuation/velocity, nonreal endpoint/Weyl data, RAQ and \(C/H\) equivalence remain open. | `claim:RAW_C_Q0_NORMALIZED_DIFFERENTIATED_ROTATING_TAIL_GAMMA1_FUNCTIONAL_HOLDS`; `concept:raw-c-panelwise-picard-sensitivity-admission`; `open:gate1-v0-raw-c-differentiated-tail-node-safe-transport`; `reading-path:gate1-v0-metacognitive-priority-to-observation` |
