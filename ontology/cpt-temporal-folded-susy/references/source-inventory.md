...
| Non-numbered Gate-1 straight-lift end-admissibility audit | Witten (2010); Banihashemi–Jacobson | `CITES`-only relative-cycle good-end and configuration-representation below-origin lapse-contour framing. Neither source selects this gravitational original cycle, supplies the Phase-39 action reduction, chooses a field lift, or proves its convergence. The exact bad subsequence, exhaustive constant-straight-line case split, direct 80-decimal controls, straight-model-class KILL, preserved local signs, and still-open Gate 1 are repository calculation-workbench evidence. No new literature source is introduced. |
