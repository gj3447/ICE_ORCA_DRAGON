...
| Non-numbered Gate-1 straight-lift, phase-band, scalar-source, and finite bosonic canonical controls | Witten (2010); Banihashemi–Jacobson; Gibbons–Hawking–Perry | Witten supplies only relative-cycle/good-end framing. Banihashemi–Jacobson's momentum-first convergence argument assumes a gauge fixing that removes the full-theory negative trace-momentum Gaussian, illustrated by \(p=q_{ij}p^{ij}=0\); it does not establish a global slicing theorem or license an unchanged real minisuperspace \(p_a\) contour. Gibbons–Hawking–Perry frames the conformal-sign obstruction but does not choose this finite contour or its determinant orientation. The exact bad subsequence, anchor-through \(\delta=qy\) case split, phase-locked branch, principal phase-band contraction, \(0<\lambda<2\) nonuniqueness family, ordered scalar nonzero-arm link, and full-bosonic opposite-sign convergence test are repository calculation-workbench evidence. The latter contradicts only absolute momentum-first convergence of unchanged real \(p_a,p_\phi\) axes under one common lateral sign; declared complex momentum rays reproduce a fiberwise Gaussian pushforward and a frozen-\(A\) flat tangent supplies the negative-arm ratio. No cited source or repository calculation derives source-to-thimble deformation, simultaneous nonlinear configuration admissibility, \(p_a\) gauge/FP/BFV completion, zero lapse, \(\lambda=1\) selection, the physical original joint cycle, global coefficient, physics or TOE. |
