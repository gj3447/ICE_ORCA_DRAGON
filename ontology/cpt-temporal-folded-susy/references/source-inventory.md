...
| Non-numbered Gate-1 straight-lift through V0 spectral-to-(m=2), densitized KL/RAQ, full-\(p\) completion, \(p=0\) boundary and zero-mode Ward controls | Witten (2010); Banihashemi–Jacobson; Gibbons–Hawking–Perry; Henneaux–Teitelboim–Vergara; García–Vergara–Urrutia; Marolf (1995, 1996); Giulini; Louko–Martinez-Pascual; Han–Thiemann; Pawłowski–Ashtekar; NIST DLMF; Halliwell (1988); Rogers; Hörmander; Van Vleck | The earlier sources retain their local framing roles. NIST DLMF supplies only the modified-Bessel/Kontorovich--Lebedev and E1 identities; Marolf, Giulini and Han–Thiemann frame single-constraint RAQ/direct-integral regularity; Louko–Martinez-Pascual makes constraint-rescaling non-equivalence a hard boundary; Pawłowski–Ashtekar is a genuine extension/gluing contrast, not a rule for this operator. Repository evidence supplies one selected densitized \(H\) realization, its cutoff boundary control, and now the regular-shell completion \(L^2(\mathbb R,dx)_+\oplus L^2(\mathbb R,dx)_-\), with \(p=0\) at \(x\to-\infty\) and no inherited origin atom. It also supplies a Ward check showing direct Berezin integration equals determinant-weighted ghost elimination while unweighted deletion is invalid. A raw-\(C\) ordering/domain or rescaling equivalence, added origin sector or branch gluing, exact normalized endpoint transform, lapse modulus/contour, absolute BFV functional measure, physical cycle, quantum gravity, physics and TOE remain open. |
