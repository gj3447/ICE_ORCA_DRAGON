...
| Metacognitive priority map and raw-\(C\) validated-transport expansion (2026-08-29) | DLMF modified-Bessel identities and 10.32.9 integral representation; Johansson's Arb; python-flint 0.9 `acb.integral`; CAPD::DynSys; VNODE-LP; Brown--McCormack--Marletta guaranteed singular Sturm--Liouville bounds; Eckhardt et al. singular Weyl--Titchmarsh theory; existing RAQ, \(S^3\), BV--BFV, clock, BO and likelihood sources | DLMF, Arb and the locked finite-integration interface frame the exact \(\lambda=0\) representation, inclusion arithmetic and analytic-tail separation used by the repository certificates. CAPD/VNODE/Brown are method candidates for the still-open node-safe compact sensitivity transport; Eckhardt et al. bounds the later nonreal spectral step. Every literature edge is `CITES`-only. None supplies this fiber's node-safe \(F_\lambda\), spectral measure, RAQ, HDA, absolute BFV, anomaly freedom or likelihood. |
