...
Every source above has `state: PRIMARY` in the local graph. That is a source classification, not a claim that the ten-item inventory exhausts the literature.
...
The other nine graph sources have stable DOI/arXiv locators but no stored local content hash or equation-level anchor. Consequently, the Phase 17 literature links document framing and scope, while the exact matrix results remain repository evidence. See [`PHASE16_BGG_SOURCE_NOTES.md`](../../../cpt_temporal_folded_susy/PHASE16_BGG_SOURCE_NOTES.md) for the detailed BGG source ledger.
