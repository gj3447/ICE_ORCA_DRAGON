...
| `source:giulini-2000-group-averaging-raq` — D. Giulini, [*Group Averaging and Refined Algebraic Quantization*](https://arxiv.org/abs/gr-qc/0003040), Nucl. Phys. B Proc. Suppl. 88 (2000) 385–388 | Primary review framing for the local RAQ/group-averaging distinction, its limitations, and auxiliary-structure dependence | Does not choose the repository's densitized ordering, Kontorovich--Lebedev normalization, physical measure, raw \(C\) domain, or \(p=0\) completion |
| `source:nist-dlmf-bessel-kl` — NIST Digital Library of Mathematical Functions, [§10.25 Modified Bessel definitions](https://dlmf.nist.gov/10.25) and [§10.43 integrals](https://dlmf.nist.gov/10.43) | Primary reference for the modified-Bessel \(K\) conventions and integral identities used in the explicitly declared Kontorovich--Lebedev spectral control | Does not select a quantum constraint, turn a transform identity into a physical rigging map, or settle the half-line edge/domain |
| `source:gradshteyn-ryzhik-8e-6-576-4` — I. S. Gradshteyn and I. M. Ryzhik, *Table of Integrals, Series, and Products*, 8th ed., formula 6.576(4) | `CITES`-only Mellin-integral formula for the full weighted \(K_{i\kappa}^2\) total in the declared raw-\(C\) \(\Gamma_1\) boundary-variation calculation | The runner checks its parameter hypotheses and conventions. The source supplies neither the repository enclosure nor an operator, boundary-extension, spectral/RAQ, physics, quantum-gravity, or TOE conclusion |
| `source:nist-dlmf-exponential-integral` — NIST Digital Library of Mathematical Functions, [§6.2 Exponential and logarithmic integrals](https://dlmf.nist.gov/6.2) | Primary reference for the E1 defining integral and its logarithmic small-argument decomposition used by the selected densitized \(p=0\) cutoff witness | Does not choose a cosmological test domain, endpoint subtraction, counterterm, sector, or canonical edge completion |
| `source:louko-martinez-pascual-2011` — J. Louko, E. Martinez-Pascual, [*Constraint rescaling in refined algebraic quantisation: momentum constraint*](https://arxiv.org/abs/1107.1092), J. Math. Phys. 52 (2011) 123504 | Primary rescaled-constraint RAQ boundary: classically equivalent rescalings can be quantum inequivalent, fail self-adjointness, or create extension/superselection ambiguity | Does not prove equivalence of the repository's raw and densitized \(V=0\) constraints or choose its \(p=0\) extension |
| `source:weyl-1910-singular-sturm-liouville` — H. Weyl, [*Über gewöhnliche Differentialgleichungen mit Singularitäten und die zugehörigen Entwicklungen willkürlicher Funktionen*](https://eudml.org/doc/158437), Math. Ann. 68 (1910) 220–269 | Primary limit-point/limit-circle endpoint alternative used after the declared raw-\(C\) fibers are put in real weighted Sturm–Liouville form | Does not choose the auxiliary measure, ordering, fiber boundary condition, measurable global extension, or physical inner product |
| `source:von-neumann-1930-self-adjoint-extensions` — J. von Neumann, [*Allgemeine Eigenwerttheorie Hermitescher Funktionaloperatoren*](https://doi.org/10.1007/BF01782338), Math. Ann. 102 (1930) 49–131 | Primary deficiency-space foundation for self-adjoint extensions of symmetric operators | Fiber indices do not select an extension or classify all full-operator extensions that may mix \(p\) fibers |
| `source:nussbaum-1964-unbounded-reduction` — A. E. Nussbaum, [*Reduction Theory for Unbounded Closed Operators in Hilbert Space*](https://doi.org/10.1215/S0012-7094-64-03103-5), Duke Math. J. 31 (1964) 33–44 | Primary reduction-theory boundary for decomposable unbounded closed operators | Does not turn fixed-fiber extension existence or a bare Borel phase into a measurable global domain/resolvent field |
| `source:lennon-1973-direct-integrals` — J. J. Lennon, [*Direct Integrals*](https://www.mscand.dk/article/download/11449/9466/30402), Math. Scand. 32 (1973) 123–132 | Primary measurable closed-fiber and direct-integral operator conditions used to scope the global raw-\(C\) extension debt | Does not classify \(p\)-mixing extensions or establish raw-\(C\)/selected-\(H\) RAQ equivalence |
| `source:han-thiemann-2010-rigging-direct-integral` — M. Han, T. Thiemann, [*On the Relation between Rigging Inner Product and Master Constraint Direct Integral Decomposition*](https://arxiv.org/abs/0911.3431v1), J. Math. Phys. 51 (2010) 092501 | Primary sufficient-condition framework relating group averaging to the absolutely continuous zero sector of a direct-integral decomposition | Its regularity and local-Abelianization hypotheses do not prove raw-\(C\)/densitized-\(H\) equivalence, add an atom at \(p=0\), or glue the two signed regular sectors |
| `source:pawlowski-ashtekar-2012-positive-lambda-lqc` — T. Pawłowski, A. Ashtekar, [*Positive cosmological constant in loop quantum cosmology*](https://arxiv.org/abs/1112.0360v1), Phys. Rev. D 85 (2012) 064001 | Primary contrasting example in which a genuine self-adjoint-extension/gluing choice occurs | It is not the present multiplication-operator completion and therefore cannot be imported as a \(p=0\) origin state or branch-gluing rule |
...
| `source:hien-2008-rapid-decay` — M. Hien, [*Periods for flat algebraic connections*](https://arxiv.org/abs/0803.3463), `arXiv:0803.3463v1` | `CITES`-only rapid-decay homology and period-pairing framework for the alternative Gate-1 chain lane | Does not supply an ICE meromorphic connection, controlled compactification/divisor, real-oriented blow-up, BFV coefficient line, source cycle, completeness proof or intersection integer |
| `source:matsubara-heo-2017-pham-comparison` — S.-J. Matsubara-Heo, [*On the rapid decay homology of F. Pham*](https://arxiv.org/abs/1705.06052), `arXiv:1705.06052v1` | `CITES`-only comparison framework for Pham-type and sheaf-theoretic rapid-decay constructions under stated hypotheses | Does not establish those hypotheses for the frozen Starobinsky action, construct the missing BFV source/orientation line, or prove the ICE candidate set complete |
...
| `source:brunetti-fredenhagen-2000-scaling-degree` — R. Brunetti, K. Fredenhagen, [*Microlocal Analysis and Interacting Quantum Field Theories: Renormalization on Physical Backgrounds*](https://arxiv.org/abs/math-ph/9903028), `arXiv:math-ph/9903028` | Theorem 5.2 is applied in the distinct frozen-m=2 flat-tangent audit after fixing one punctured branch: at omega=d=1, same-scaling-degree extensions retain a c-delta parameter | The consumed full-q scalar runner remains invalid and supplied no application evidence; the theorem neither chooses C/abs(N) versus C/N glue nor selects a physical contact coefficient |
| `source:dang-2014-distribution-extension` — N. V. Dang, [*Extension of distributions, scalings and renormalization of QFT on Riemannian manifolds*](https://arxiv.org/abs/1411.3670), `arXiv:1411.3670` | Primary Hadamard finite-part and singular-set-supported distribution-extension framework used as a theorem guard for the frozen-m=2 audit | Classifies how extensions can differ after the punctured distribution is fixed; it supplies no model-specific glue, contact coefficient, determinant orientation or physical cycle |
...
| `source:mcguigan-2023-starobinsky-morse` | Prior Starobinsky--Morse/SUSYQM connection for the declared-ordering fiber discriminator | Does not establish the repository's Laplace--Beltrami ordering, all-scale whole-fiber spectrum, source domain, or physical interpretation |
...
| Non-numbered Gate-1 straight-lift through V0 spectral-to-(m=2), densitized KL/RAQ, full-\(p\) completion, raw-\(C\) fibers, endpoint support, finite BFV orientation and \(S^3\) scalar projection controls | Witten (2010); Banihashemi–Jacobson; Gibbons–Hawking–Perry; Henneaux–Teitelboim–Vergara; García–Vergara–Urrutia; Marolf (1995, 1996); Giulini; Louko–Martinez-Pascual; Han–Thiemann; Pawłowski–Ashtekar; NIST DLMF; Halliwell (1988); Rogers; Hörmander; Van Vleck; Quillen; Bismut–Freed; Govaerts–Scholtz; Kamenshchik–Lyakhovich; Halliwell–Hawking | The sources retain framing roles only. Repository evidence supplies one selected densitized \(H\) realization and two-branch regular completion; one declared raw-\(C\) fixed-fiber extension debt; an exact selected-\(H\) map to \(c<3p^2\) together with a \(p\)-preserving unrestricted-target obstruction; one finite positive-\(\lambda\) relative odd orientation; and a unit-\(S^3\), \(L=2\) scalar truncation-remainder ledger. None of the cited papers supplies those repository matrices or checks. A measurable global raw domain and rescaling equivalence, old-\(W\) endpoint transform, absolute BFV measure, ADM/HDA closure, anomaly freedom, empirical map, quantum gravity, physics and TOE remain open. |
| Raw-C declared constant-boundary direct-integral extension and closed-S3 ADM convention audit (2026-08-28) | Nussbaum (1964); Lennon (1973); Gitman–Smirnov–Tyutin–Voronov; Weyl (1910); Teitelboim (1973); Halliwell–Hawking (1985); NIST DLMF | `CITES`-only theorem and convention boundaries. Repository evidence constructs one declared p-preserving measurable decomposable raw-C extension and fixes a homogeneous closed-FRW ADM-to-raw-C, zonal-scalar and longitudinal-coordinate baseline. These sources do not physically select the boundary line, establish raw-C RAQ/equivalence, supply full SVT or linear constraints, or prove cubic HDA/Jacobi, BFV, physics or TOE results. |
| Six-bridge literature expansion and executable status cross-links (2026-08-28) | RAQ/direct integrals and domains; Egorov/FIO theory; BFV/BV-BFV, Gribov and determinant-line theory; closed-space perturbative Dirac/BRST constructions; relational clocks and decoherence; CLASS/Cobaya plus Planck, ACT DR6 and SPT-3G D1 | All literature links are `CITES`-only and are indexed by the six-bridge map and version/hash manifest. The map now cross-links three separately committed workbench controls for restricted endpoint support, finite relative Pfaffian orientation and \(S^3\) projection remainder; these are not claims sourced from the papers. No raw-\(C\leftrightarrow H\) equivalence, absolute BFV measure, HDA closure, anomaly-free inhomogeneous quantization, empirical fit, quantum-gravity theory or TOE result is established. Local PDF binaries are reconstructable and ignored. |
| Metacognitive priority map and raw-\(C\) Green-endpoint expansion (2026-08-29) | DLMF modified-Bessel identities and 10.32.9 integral representation; Johansson's Arb; python-flint 0.9 `acb.integral`; CAPD::DynSys; VNODE-LP; Brown--McCormack--Marletta guaranteed singular Sturm--Liouville bounds; Eckhardt et al. singular Weyl--Titchmarsh theory; existing RAQ, \(S^3\), BV--BFV, clock, BO and likelihood sources | DLMF, Arb and the locked finite-integration interface frame the exact \(\lambda=0\) representation, inclusion arithmetic, six-segment Green endpoint construction and analytic-tail separation used by the repository certificates. CAPD/VNODE/Brown remain optional methods for an explicit compact-state decomposition or the nonzero-\(\lambda\) boundary problem; the primary P1 gap is now the minus-end \(\Gamma_1\) functional and its left-boundary contribution. Eckhardt et al. bounds the later nonreal spectral step. Every literature edge is `CITES`-only. None supplies this fiber's declared \(F_\lambda\), spectral measure, RAQ, HDA, absolute BFV, anomaly freedom or likelihood. |
