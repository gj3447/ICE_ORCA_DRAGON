...
| Non-numbered Gate-1 straight-lift, phase-band, scalar-source, finite bosonic canonical, trace-gauge, V0 endpoint and componentwise off-shell controls | Witten (2010); Banihashemi–Jacobson; Gibbons–Hawking–Perry; Henneaux–Teitelboim–Vergara; Marolf (1996); Halliwell (1988); Rogers | Witten supplies only relative-cycle/good-end framing. Banihashemi–Jacobson supplies the local trace-gauge/FP premise before a lower-lateral momentum-first treatment and spatially flat benchmark, not a global slicing theorem or this curved closed-FRW chart. Henneaux–Teitelboim–Vergara supplies the general endpoint-transformation/improved-action framework; Marolf and Halliwell bound gauge-fixed/full-lapse minisuperspace readings; Rogers separates local FP nonvanishing from global BFV/Gribov admissibility. Gibbons–Hawking–Perry frames the conformal-sign obstruction. Repository evidence supplies the exact bad subsequence, affine class, scalar-source link, opposite-sign bosonic convergence obstruction, homogeneous trace pair and FP horizon, local reduction, weak \(\Phi_*\), shell one-form, finite endpoint hit, local FP measure, time-dependent same-orbit control, unique positive off-shell root, mixed generator, componentwise Darboux chart and endpoint potential. It contradicts only the unchanged-source static append, supports one classical local on-shell action, and now supports its classical extension on \(\mathcal U_+=\{p>0,R>0\}\). Other components, a global atlas, normalized quantum endpoint states, replacement ghost/BFV source, old fixed-\(a\) equality, full-real-lapse distributional \(\delta(C)\) kernel, zero lapse, physical original joint cycle, global coefficient, physics and TOE all remain open or null. |
