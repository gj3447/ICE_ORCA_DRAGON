...
| Non-numbered Gate-1 straight-lift through V0 spectral-to-(m=2), densitized KL/RAQ and zero-mode Ward controls | Witten (2010); Banihashemi–Jacobson; Gibbons–Hawking–Perry; Henneaux–Teitelboim–Vergara; García–Vergara–Urrutia; Marolf (1995, 1996); Giulini; Louko–Martinez-Pascual; NIST DLMF; Halliwell (1988); Rogers; Hörmander; Van Vleck | The earlier sources retain their local framing roles. NIST DLMF supplies only the modified-Bessel/Kontorovich--Lebedev identities; Marolf and Giulini frame single-constraint RAQ; Louko–Martinez-Pascual makes constraint-rescaling non-equivalence a hard boundary. Repository evidence now also supplies one selected densitized \(H\) spectral realization with the positive \(p>0\) measure \(dp/(2\sqrt6\hbar p)\), and a Ward check showing direct Berezin integration equals determinant-weighted ghost elimination while unweighted deletion is invalid. It still does not supply a raw-\(C\) ordering/domain or rescaling equivalence, the \(p=0\) edge, an exact normalized endpoint transform, a lapse modulus/contour, an absolute BFV functional measure, a physical cycle, quantum gravity, physics or TOE. |
