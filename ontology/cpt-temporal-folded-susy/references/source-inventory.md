...
| Non-numbered Gate-1 straight-lift audit, phase-locked branch, and affine phase-band classification | Witten (2010); Banihashemi–Jacobson | `CITES`-only relative-cycle good-end framing and, in Banihashemi–Jacobson's stated full-real-lapse construction and momentum-first integration order, below-origin lapse-contour framing. Neither source selects the finite Phase-39 bypass or this gravitational original cycle, supplies the Phase-39 action reduction, chooses a field lift, or proves its convergence. The exact bad subsequence, anchor-through \(\delta=qy\) case split, direct 80-decimal controls, scoped straight KILL, phase-locked two-good-end construction, and follow-up proof that the strict full-rate-good continuous horizontal affine tails form one principal reduced class are repository calculation-workbench evidence. The \(0<\lambda<2\) counterfamily shows that goodness does not select the phase-locked representative. Phase 30's literal real-field arm restriction is the \(\lambda=0\) oscillatory boundary in this fixed-\(a\) scalar-tail test; extending \(b=0\) across the complex cap is a candidate lift, and comparison with a fully specified \(\Gamma_\lambda\) limiting family still requires distributional phase-space/orientation analysis. Original-cycle, scale-factor, regulator-removal, Stokes/census, and global boundaries stay open. No new literature source is introduced. |
