...
import { researchEpisodeCommand, researchFeedbackCommand, researchStateCommand } from "../research-engine/commands.ts"
...
const state = Command.make("state", { json }, ({ json }) => researchStateCommand("status", json))
const graph = Command.make("graph", { json }, ({ json }) => researchStateCommand("graph", json))
...
const cell = Command.make("cell", {
  stage: Args.choice([
    ...
    ["synthesize", "synthesize"], ["compute", "compute"]
  ] as const, { name: "stage" })
...
export const researchContextCommand = Command.make("research", {}, () =>
  Console.log("Use `ice research run <question>`, `plan`, `state`, `graph`, or `feedback <episode>`."))
  .pipe(Command.withDescription("HSWM research execution and USL reference context"),
    Command.withSubcommands([run, plan, state, graph, feedback, status, prepare, cell]))
