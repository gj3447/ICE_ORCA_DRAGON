...
const target = Options.text("target").pipe(
  Options.withDescription("required target: intuition::topic:id or cpt::open:id")
)
const limit = Options.integer("limit").pipe(
  Options.withDefault(8),
  Options.withDescription("maximum canonical context hits when the target is canonical (1-50)")
)
...
const search = Command.make(
  ...
  Command.withDescription(
    "retrieve source-backed lenses by sidecar topic or exact canonical open-problem context"
  )
)
